...
VOID VmxGetSegmentDescriptorFromSelector(PVMX_SEGMENT_DESCRIPTOR VmxSegmentDescriptor, SEGMENT_DESCRIPTOR_REGISTER_64 GdtRegister, SEGMENT_SELECTOR SegmentSelector, BOOL ClearRPL)
{
	 ...
	 */
	SEGMENT_DESCRIPTOR_64* OsSegmentDescriptor;
	 ...
	 */
	OsSegmentDescriptor = (SEGMENT_DESCRIPTOR_64*)(((UINT64)GdtRegister.BaseAddress) + (SegmentSelector.Index << 3));
...
/*
* Calls SetupVmcsGuestSegment with encoded VMCS values corresponding to that segment.
*/
...
*/
IA32_VMX_PINBASED_CTLS_REGISTER SetupVmcsControlPinBased(PVMM_PROCESSOR_CONTEXT CpuContext) {
	 ...
	 */
	if (CpuContext->GlobalContext->VmxCapabilities.VmxControls == 1)
	{
	...
	Register.Flags = EncodeMustBeBits(Register.Flags, ConfigMSR);
 ...
 */
IA32_VMX_PROCBASED_CTLS_REGISTER SetupVmcsControlProcessor(PVMM_PROCESSOR_CONTEXT CpuContext)
{
 ...
 */
IA32_VMX_PROCBASED_CTLS2_REGISTER SetupVmcsControlSecondaryProcessor(PVMM_PROCESSOR_CONTEXT Context)
{
 ...
 */
IA32_VMX_EXIT_CTLS_REGISTER SetupVmcsControlVmExit(PVMM_PROCESSOR_CONTEXT CpuContext)
{
 ...
 */
IA32_VMX_ENTRY_CTLS_REGISTER SetupVmcsControlVmEntry(PVMM_PROCESSOR_CONTEXT CpuContext)
{
...
SIZE_T SetupVmcsControlFields(PVMM_PROCESSOR_CONTEXT Context) {
	SIZE_T VmError = 0;

	/////////////////////////////// Pin-based Control ///////////////////////////////
	VmxVmwriteFieldFromRegister(VMCS_CTRL_PIN_BASED_VM_EXECUTION_CONTROLS, SetupVmcsControlPinBased(Context));

	/////////////////////////////// Processor-Based VM-Execution Controls ///////////////////////////////
	VmxVmwriteFieldFromRegister(VMCS_CTRL_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, SetupVmcsControlProcessor(Context));
	...
	/////////////////////////////// VM-Exit Controls ///////////////////////////////
	VmxVmwriteFieldFromRegister(VMCS_CTRL_VMEXIT_CONTROLS, SetupVmcsControlVmExit(Context));
	...
	/////////////////////////////// VM-Entry Controls ///////////////////////////////
	VmxVmwriteFieldFromRegister(VMCS_CTRL_VMENTRY_CONTROLS, SetupVmcsControlVmEntry(Context));
	...
	VmxVmwriteFieldFromRegister(VMCS_CTRL_SECONDARY_PROCESSOR_BASED_VM_EXECUTION_CONTROLS, SetupVmcsControlSecondaryProcessor(Context));
	...
	VmxVmwriteFieldFromRegister(VMCS_CTRL_CR0_READ_SHADOW, Context->SpecialRegistersContext.ControlRegister0);
	VmxVmwriteFieldFromRegister(VMCS_CTRL_CR4_READ_SHADOW, Context->SpecialRegistersContext.ControlRegister4);
 ...
 */
SIZE_T SetupVmcsGuestArea(PVMM_PROCESSOR_CONTEXT Context, SIZE_T GuestRIP, SIZE_T GuestRSP) {
	SIZE_T VmError = 0;
	 ...
	 */
	Registers = &Context->RegistersContext;

	/*
	 * Special registers of the host, such as control registers (CR0, CR4)
	 */
	SpecialRegisters = &Context->SpecialRegistersContext;
...
*/
SIZE_T SetupVmcsHostArea(PVMM_PROCESSOR_CONTEXT CpuContext, SIZE_T HostRIP, SIZE_T HostRSP)
{
	SIZE_T VmError = 0;
	PIA32_SPECIAL_REGISTERS SpecialRegisters;
	PREGISTER_CONTEXT Registers;
	 ...
	 * Registers as they were when we began setup. Used to get segment selector values.
	 */
	Registers = &CpuContext->RegistersContext;

	/*
	 * Special registers of the host, such as control registers (CR0, CR4)
	 */
	SpecialRegisters = &CpuContext->SpecialRegistersContext;
	...
	VmxVmwriteFieldFromImmediate(VMCS_HOST_CR3, CpuContext->GlobalContext->SystemDirectoryTableBase);
...
BOOL SetupVmcsDefaults(PVMM_PROCESSOR_CONTEXT Context, SIZE_T HostRIP, SIZE_T HostRSP, SIZE_T GuestRIP, SIZE_T GuestRSP) {
	 ...
	 */
	OsCaptureContext(&Context->RegistersContext);
	 ...
	 * running host.
	 */
	ArchCaptureSpecialRegisters(&Context->SpecialRegistersContext);

	// Setup all of the control fields of the VMCS
	VmError |= SetupVmcsControlFields(Context);
	HxTLog("SetupVmcsControlFields: VmError = %i\n", VmError);
	if (VmError != 0)
	{
		HxTLog("SetupVmcsControlFields: VmError = %i\n", VmError);
		return FALSE;
	...
	VmError |= SetupVmcsGuestArea(Context, GuestRIP, GuestRSP);
	if (VmError != 0)
	{
		HxTLog("SetupVmcsGuestArea: VmError = %i\n", VmError);
		return FALSE;
	}

	// Setup the host area of the VMCS
	VmError |= SetupVmcsHostArea(Context, HostRIP, HostRSP);
	if (VmError != 0)
	{
		HxTLog("SetupVmcsHostArea: VmError = %i\n", VmError);
		return FALSE;
	}
