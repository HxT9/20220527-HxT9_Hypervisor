...
{
	static WCHAR BlockedFileName[] = L"hxt9.txt";
...
BOOL EptCheckFeatures()
{
	...
	{
		HxTLog("Processor does not support AdvancedVmexitEptViolationsInformation!\n");
	}

	if (!MTRRDefType.MtrrEnable)
	{
		HxTLog("MTRR Dynamic Ranges not supported.\n");
		return FALSE;
	}

	HxTLog("EptCheckFeatures: All EPT features present.\n");
	return TRUE;
}
...
BOOL EptBuildMTRRMap(PVMM_CONTEXT Context)
{
	...
	HxTLog("EPT: Number of dynamic ranges: %d\n", MTRRCap.VariableRangeCount);
	...
	for (CurrentRegister = 0; CurrentRegister < MTRRCap.VariableRangeCount; CurrentRegister++)
	{
		...
		if (CurrentPhysMask.Valid)
		{
			...
			Descriptor = &Context->MemoryRanges[Context->EnabledMemoryRanges++];
			...
			if (Descriptor->MemoryType == MEMORY_TYPE_WRITE_BACK)
			{
				...
				Context->EnabledMemoryRanges--;
			}
			HxTLog("MTRR Range: Base=0x%llX End=0x%llX Type=0x%X\n", Descriptor->PhysicalBaseAddress, Descriptor->PhysicalEndAddress, Descriptor->MemoryType);
		}
	}

	HxTLog("Total MTRR Ranges Committed: %d\n", Context->EnabledMemoryRanges);
...
BOOL EptGlobalInitialize(PVMM_CONTEXT Context) {
	...
	{
		HxTLog("Could not build MTRR memory map.\n");
		return FALSE;
	}
 ...
 */
VOID EptSetupPML2Entry(PVMM_CONTEXT Context, EPDE_2MB* NewEntry, SIZE_T PageFrameNumber)
{
	...
	/* For each MTRR range */
	for (CurrentMtrrRange = 0; CurrentMtrrRange < Context->EnabledMemoryRanges; CurrentMtrrRange++)
	{
		/* If this page's address is below or equal to the max physical address of the range */
		if (AddressOfPage <= Context->MemoryRanges[CurrentMtrrRange].PhysicalEndAddress)
		{
			/* And this page's last address is above or equal to the base physical address of the range */
			if ((AddressOfPage + SIZE_2_MB - 1) >= Context->MemoryRanges[CurrentMtrrRange].PhysicalBaseAddress)
			{
				 ...
				 */
				TargetMemoryType = Context->MemoryRanges[CurrentMtrrRange].MemoryType;
				//UtilLogDebug("0x%X> Range=%llX -> %llX | Begin=%llX End=%llX", PageFrameNumber, AddressOfPage, AddressOfPage + SIZE_2_MB - 1, GlobalContext->MemoryRanges[CurrentMtrrRange].PhysicalBaseAddress, GlobalContext->MemoryRanges[CurrentMtrrRange].PhysicalEndAddress);
...
PVMM_EPT_PAGE_TABLE EptAllocateAndCreateIdentityPageTable(PVMM_CONTEXT Context)
{
	PVMM_EPT_PAGE_TABLE PageTable;
	EPDPTE RWXTemplate;
	EPDE_2MB PML2EntryTemplate;
	...
	if (PageTable == NULL)
	{
		HxTLog("EptCreatePageTable: Failed to allocate memory for PageTable.\n");
		return NULL;
	}
	...
	for (EntryGroupIndex = 0; EntryGroupIndex < VMM_EPT_PML3E_COUNT; EntryGroupIndex++)
	{
		...
		for (EntryIndex = 0; EntryIndex < VMM_EPT_PML2E_COUNT; EntryIndex++)
		{
			...
			EptSetupPML2Entry(Context, &PageTable->PML2[EntryGroupIndex][EntryIndex], (EntryGroupIndex * VMM_EPT_PML2E_COUNT) + EntryIndex);
		}
	}
...
BOOL EptLogicalProcessorInitialize(PVMM_PROCESSOR_CONTEXT ProcessorContext) {
	...
	PageTable = EptAllocateAndCreateIdentityPageTable(ProcessorContext->GlobalContext);
	if (PageTable == NULL)
	{
		HxTLog("Unable to allocate memory for EPT!\n");
		return FALSE;
	}
	 ...
	 */
	if (!EptAddPageHook(ProcessorContext, (PVOID)NtCreateFile, (PVOID)NtCreateFileHook, (PVOID*)&NtCreateFileOrig))
	{
		HxTLog("Failed to build page hook for NtCreateFile");
		EptFreeLogicalProcessorContext(ProcessorContext);
		return FALSE;
	}
 ...
 */
EPDE_2MB* EptGetPml2Entry(PVMM_PROCESSOR_CONTEXT ProcessorContext, SIZE_T PhysicalAddress)
{
 ...
 */
EPTE* EptGetPml1Entry(PVMM_PROCESSOR_CONTEXT ProcessorContext, SIZE_T PhysicalAddress)
{
	SIZE_T Directory, DirectoryPointer, PML4Entry;
	EPDE_2MB* PML2;
	EPTE* PML1;
	 ...
	 */
	PML2Pointer = (EPDE*)PML2;

	/* If it is, translate to the PML1 pointer */
	PML1 = (EPTE*)OsPhysicalToVirtual(PML2Pointer->PageFrameNumber * PAGE_SIZE);

	if (!PML1)
	{
		HxTLog("Failed to get PML1 entry: Translating PA:%p to VA returned NULL.", PML2Pointer->PageFrameNumber * PAGE_SIZE);
		return NULL;
	}
 ...
 */
BOOL EptSplitLargePage(PVMM_PROCESSOR_CONTEXT ProcessorContext, SIZE_T PhysicalAddress)
{
	PVMM_EPT_DYNAMIC_SPLIT NewSplit;
	EPTE EntryTemplate;
	SIZE_T EntryIndex;
	EPDE_2MB* TargetEntry;
	EPDE NewPointer;

	HxTLog("Splitting large page @ PA:%p", PhysicalAddress);

	/* Find the PML2 entry that's currently used*/
	TargetEntry = EptGetPml2Entry(ProcessorContext, PhysicalAddress);
	if (!TargetEntry)
	{
		HxTLog("EptSplitLargePage: Invalid physical address.\n");
		return FALSE;
	}
...
BOOL EptHookInstructionMemory(PVMM_EPT_PAGE_HOOK Hook, PVOID TargetFunction, PVOID HookFunction, PVOID* OrigFunction)
{
	...
	if (!Hook->Trampoline)
	{
		HxTLog("Could not allocate trampoline function buffer.\n");
		return FALSE;
	}
	...
	*OrigFunction = Hook->Trampoline;
	...
	EptHookWriteAbsoluteJump(&Hook->FakePage[OffsetIntoPage], (SIZE_T)HookFunction);
...
BOOL EptAddPageHook(PVMM_PROCESSOR_CONTEXT ProcessorContext, PVOID TargetFunction, PVOID HookFunction, PVOID* OrigFunction)
{
	...
	if (!PhysicalAddress)
	{
		HxTLog("EptAddPageHook: Target address could not be mapped to physical memory!\n");
		return FALSE;
	}
	...
	if (!NewHook)
	{
		HxTLog("EptAddPageHook: Could not allocate memory for new hook.\n");
		return FALSE;
	}
	 ...
	 */
	if (!EptSplitLargePage(ProcessorContext, PhysicalAddress))
	{
		HxTLog("EptAddPageHook: Could not split page for address 0x%llX.\n", PhysicalAddress);
	...
	NewHook->TargetPage = EptGetPml1Entry(ProcessorContext, PhysicalAddress);

	/* Ensure the target is valid. */
	if (!NewHook->TargetPage)
	{
		HxTLog("EptAddPageHook: Failed to get PML1 entry for target address.\n");
		OsFreeNonpagedMemory(NewHook);
		return FALSE;
	}
	...
	if (!EptHookInstructionMemory(NewHook, TargetFunction, HookFunction, OrigFunction))
	{
		HxTLog("EptAddPageHook: Could not build hook.\n");
		OsFreeNonpagedMemory(NewHook);
		return FALSE;
	}
	...
	VMX_EXIT_QUALIFICATION_EPT_VIOLATION ViolationQualification)
{
	...
	HxTLog("Hooked page had invalid page swapping logic?!\n");
...
VOID ExitHandleEptViolation(PVMM_PROCESSOR_CONTEXT ProcessorContext, PVMEXIT_CONTEXT ExitContext)
{
	...
	HxTLog("EPT Violation => 0x%llX\n", ExitContext->GuestPhysicalAddress);

	if (ExitHandlePageHookExit(ProcessorContext, ExitContext, ViolationQualification))
	{
		// Handled by page hook code.
		return;
	}

	HxTLog("Unexpected EPT violation!\n");
