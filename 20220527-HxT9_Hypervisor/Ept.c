...
BOOL g_ExecuteOnlySupport = FALSE;
...
BOOL EptCheckFeatures() {
    ...
    {
        HxTLog("The processor doesn't report advanced VM-exit information for EPT violations");
    }

    if (!VpidRegister.ExecuteOnlyPages)
    {
        g_ExecuteOnlySupport = FALSE;
        HxTLog("The processor doesn't support execute-only pages");
    }
    else
    {
    ...
    {
        HxTLog("MTRR dynamic ranges are not supported");
        return FALSE;
    }

    HxTLog("All EPT features are present");
 ...
 * @return BOOLEAN
 */
BOOL EptBuildMtrrMap(PVMM_CONTEXT Context)
{
    ...
    for (CurrentRegister = 0; CurrentRegister < MTRRCap.VariableRangeCount; CurrentRegister++)
    {
        ...
        if (CurrentPhysMask.Valid)
        {
            ...
            //
            Descriptor = &Context->EptState->MemoryRanges[Context->EptState->NumberOfEnabledMemoryRanges++];
            ...
            if (Descriptor->MemoryType == MEMORY_TYPE_WRITE_BACK)
            {
                ...
                //
                Context->EptState->NumberOfEnabledMemoryRanges--;
            }
            HxTLog("MTRR Range: Base=0x%llx End=0x%llx Type=0x%x", Descriptor->PhysicalBaseAddress, Descriptor->PhysicalEndAddress, Descriptor->MemoryType);
        }
    }

    HxTLog("Total MTRR ranges committed: 0x%x", Context->EptState->NumberOfEnabledMemoryRanges);
 ...
 * @return VOID
 */
VOID EptSetupPML2Entry(PEPT_PML2_ENTRY NewEntry, SIZE_T PageFrameNumber, PVMM_CONTEXT Context)
{
    ...
    // For each MTRR range
    //
    for (CurrentMtrrRange = 0; CurrentMtrrRange < Context->EptState->NumberOfEnabledMemoryRanges; CurrentMtrrRange++)
    {
        //
        // If this page's address is below or equal to the max physical address of the range
        //
        if (AddressOfPage <= Context->EptState->MemoryRanges[CurrentMtrrRange].PhysicalEndAddress)
        {
            //
            // And this page's last address is above or equal to the base physical address of the range
            //
            if ((AddressOfPage + SIZE_2_MB - 1) >= Context->EptState->MemoryRanges[CurrentMtrrRange].PhysicalBaseAddress)
            {
                //
                ...
                //
                TargetMemoryType = Context->EptState->MemoryRanges[CurrentMtrrRange].MemoryType;
 ...
 */
PVMM_EPT_PAGE_TABLE EptInitializeIdentityPageTable(PVMM_CONTEXT Context)
{
    PVMM_EPT_PAGE_TABLE PageTable;
    EPT_PML3_POINTER    PML3EntryTemplate;
    ...
    if (PageTable == NULL)
    {
        HxTLog("Failed to allocate memory for PageTable");
        return NULL;
    }
    ...
    //
    PML3EntryTemplate.ReadAccess = 1;
    PML3EntryTemplate.WriteAccess = 1;
    PML3EntryTemplate.ExecuteAccess = 1;

    //
    // Copy the template into each of the 512 PML3 entry slots
    //
    __stosq((SIZE_T*)&PageTable->PML3[0], PML3EntryTemplate.Flags, VMM_EPT_PML3E_COUNT);
    ...
    for (EntryGroupIndex = 0; EntryGroupIndex < VMM_EPT_PML3E_COUNT; EntryGroupIndex++)
    {
        ...
        for (EntryIndex = 0; EntryIndex < VMM_EPT_PML2E_COUNT; EntryIndex++)
        {
            ...
            //
            EptSetupPML2Entry(&PageTable->PML2[EntryGroupIndex][EntryIndex], (EntryGroupIndex * VMM_EPT_PML2E_COUNT) + EntryIndex, Context);
        }
    }
...
EPT_POINTER EptInitializeEptPointer(PVMM_CONTEXT Context) {
    EPT_POINTER EPTP = { 0 };

    EPTP.MemoryType = MEMORY_TYPE_WRITE_BACK;
    EPTP.EnableAccessAndDirtyFlags = FALSE;
    EPTP.PageWalkLength = 3;
    EPTP.PageFrameNumber = (SIZE_T)OsVirtualToPhysical(Context->EptState->EptPageTable->PML4) / PAGE_SIZE;

    return EPTP;
}

BOOL EptInitialize(PVMM_CONTEXT Context) {
	Context->EptState = OsAllocateNonpagedMemory(sizeof(EPT_STATE));
	if (!Context->EptState) {
		HxTLog("Can't allocate EptState. Insufficient memory");
		return FALSE;
	}

	OsZeroMemory(Context->EptState, sizeof(EPT_STATE));

    InitializeListHead(&Context->EptState->DynamicSplitList);
	InitializeListHead(&Context->EptState->HookedPagesList);

    if (!EptCheckFeatures()) {
        return FALSE;
    }

    if (!EptBuildMtrrMap(Context)) {
        return FALSE;
    }

    Context->EptState->EptPageTable = EptInitializeIdentityPageTable(Context);
    if (!Context->EptState->EptPageTable) {
        return FALSE;
    }
    Context->EptState->EptPointer = EptInitializeEptPointer(Context);
 ...
 */
PEPT_PML2_ENTRY EptGetPml2Entry(PVMM_CONTEXT Context, SIZE_T PhysicalAddress)
{
    ...
    PML2 = &Context->EptState->EptPageTable->PML2[DirectoryPointer][Directory];
    return PML2;
}
...
BOOLEAN EptSplitLargePage(PVMM_CONTEXT Context, SIZE_T PhysicalAddress)
{
    ...
    //
    TargetEntry = EptGetPml2Entry(Context, PhysicalAddress);
    if (!TargetEntry)
    {
        HxTLog("EptSplitLargePage, an invalid physical address passed");
        return FALSE;
    }
    ...
    if (!NewSplit)
    {
        HxTLog("EptSplitLargePage: Failed to allocate dynamic split memory.\n");
        return FALSE;
    }
    ...
    InsertHeadList(&Context->EptState->DynamicSplitList, &NewSplit->DynamicSplitList);
...
/**
 * @brief INVEPT wrapper in VmxDefs.asm
 * @param Type
 * @param Descriptors
...
VOID EptSetPML1AndInvalidateTLB(PVMM_CONTEXT Context, PEPT_PML1_ENTRY EntryAddress, EPT_PML1_ENTRY EntryValue, INVEPT_TYPE InvalidationType)
{
    ...
    {
        EptInveptSingleContext(Context->EptState->EptPointer.Flags);
    }
    ...
    {
        HxTLog("EptSetPML1AndInvalidateTLB, invald invalidation parameter");
    }
 ...
 */
BOOLEAN EptHandlePageHookExit(PVMM_PROCESSOR_CONTEXT ProcessorContext, PVMEXIT_CONTEXT ExitContext, VMX_EXIT_QUALIFICATION_EPT_VIOLATION ViolationQualification)
{
    BOOLEAN     IsHandled = FALSE;
    PLIST_ENTRY TempList = 0;

    TempList = &ProcessorContext->GlobalContext->EptState->HookedPagesList;
    while (&ProcessorContext->GlobalContext->EptState->HookedPagesList != TempList->Flink)
    {
        TempList = TempList->Flink;
        PEPT_HOOKED_PAGE_DETAIL HookedEntry = CONTAINING_RECORD(TempList, EPT_HOOKED_PAGE_DETAIL, PageHookList);

        if (HookedEntry->PhysicalBaseAddress == PAGE_ALIGN(ExitContext->GuestPhysicalAddress))
        {
            //
            // We found an address that matches the details
            //
            if (!(ViolationQualification.EptExecutable || ViolationQualification.EptExecutableForUserMode) && ViolationQualification.ExecuteAccess) {
                /* Swap out the non-executable page and swap in the executable page */
                EptSetPML1AndInvalidateTLB(ProcessorContext->GlobalContext, HookedEntry->EntryAddress, HookedEntry->ExecuteEntry, InveptSingleContext);
            ...
            if ((ViolationQualification.EptExecutable || ViolationQualification.EptExecutableForUserMode) && (ViolationQualification.ReadAccess || ViolationQualification.WriteAccess)) {
                /* Swap out the non-executable page and swap in the executable page */
                EptSetPML1AndInvalidateTLB(ProcessorContext->GlobalContext, HookedEntry->EntryAddress, HookedEntry->RWEntry, InveptSingleContext);
...
BOOLEAN EptHandleEptViolation(PVMM_PROCESSOR_CONTEXT ProcessorContext, PVMEXIT_CONTEXT ExitContext)
{
    ...
    if (EptHandlePageHookExit(ProcessorContext, ExitContext, ViolationQualification))
    {
        //
    ...
    HxTLog("EptHandleEptViolation, unexpected EPT violation\n");
